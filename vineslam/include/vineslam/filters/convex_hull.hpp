...
struct ConvexHull
{
                                    ...
                                    Point& isct)
  {
    ...
    else
    {
      ...
      bool online1 = ((std::min(l1p1.x_, l1p2.x_) < x || Const::isEqual(std::min(l1p1.x_, l1p2.x_), x)) &&
                      (std::max(l1p1.x_, l1p2.x_) > x || Const::isEqual(std::max(l1p1.x_, l1p2.x_), x)) &&
                      (std::min(l1p1.y_, l1p2.y_) < y || Const::isEqual(std::min(l1p1.y_, l1p2.y_), y)) &&
                      (std::max(l1p1.y_, l1p2.y_) > y || Const::isEqual(std::max(l1p1.y_, l1p2.y_), y)));
      bool online2 = ((std::min(l2p1.x_, l2p2.x_) < x || Const::isEqual(std::min(l2p1.x_, l2p2.x_), x)) &&
                      (std::max(l2p1.x_, l2p2.x_) > x || Const::isEqual(std::max(l2p1.x_, l2p2.x_), x)) &&
                      (std::min(l2p1.y_, l2p2.y_) < y || Const::isEqual(std::min(l2p1.y_, l2p2.y_), y)) &&
                      (std::max(l2p1.y_, l2p2.y_) > y || Const::isEqual(std::max(l2p1.y_, l2p2.y_), y)));
  ...
  static void getIntersectionPoints(const Point& l1p1, const Point& l1p2, const SemiPlane& poly,
                                    std::vector<Point>& isct)
  {
  ...
  static bool polygonIntersection(const SemiPlane& S1, const SemiPlane& S2, std::vector<Point>& isct)
  {
    ...
    for (int i = 0, next = 1; i < static_cast<int>(S1.extremas_.size());
         i++, next = (i + 1 == static_cast<int>(S1.extremas_.size())) ? 0 : i + 1)
    {
  ...
  static bool process(const Plane& plane, SemiPlane& semi_plane)
  {
    // To find orientation of ordered triplet (p, q, r).
    // The function returns following values
    // 0 --> p, q and r are colinear

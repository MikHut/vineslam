...
struct pose {
  ...
  pose(const std::vector<pose>& poses)
  {
    ...
    for (const auto& pose : poses) {
      gauss.stdX += pow(pose.x - mean.x, 2);
      gauss.stdY += pow(pose.y - mean.y, 2);
    }
  ...
  void setDist(const ellipse& other) { dist = other; }
  ...
  ellipse getDist() const { return dist; }
